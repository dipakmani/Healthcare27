...
MAX_VISITS_PER_PATIENT = 4
TOTAL_ROWS = TOTAL_PATIENTS * MAX_VISITS_PER_PATIENT
CHUNK_SIZE = 50_000
...
def calc_age(dob: date, on_date: date) -> int:
    return max(0, on_date.year - dob.year - ((on_date.month, on_date.day) < (dob.month, dob.day)))
...
# ---------------------------
COLUMNS = [
    "visitid","admission_date","discharge_date","patientid","patient_fullname","patientgender","age","patientdob","patientaddress",
    "patientcity","patientstate","patientcountry","patientpostalcode","patient_satisfactionsxore","patient_bloodgroup","patientwaittime",
...
def generate_full():
    ...
    # Create patient visit plan with 2-4 visits per patient
    patient_visit_plan = []
    for patient in patients:
        num_visits = random.randint(MIN_VISITS_PER_PATIENT, MAX_VISITS_PER_PATIENT)
        patient_visit_plan.extend([patient]*num_visits)
    
    random.shuffle(patient_visit_plan)
    
    for patient in patient_visit_plan:
    ...
    print(f"ℹ️ Unique patients: {len(patients):,}")
